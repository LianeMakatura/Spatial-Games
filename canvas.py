# Heavily borrowed from CS1lib, written by Devin Balkcom, Hany Farid, and Tom Cormen
...
print("Imported canvas, version " + __version__)
...
class CS1Canvas(QWidget):
    ...
    def __init__(self, draw_fn, data, window_x, window_y, width, height, title, framerate,
        ...
        self.timer = None # for later

    def c_start_timer(self):
        self.timer = QTimer()
...
def set_canvas(draw_func=noop, data=None, framerate=40,
    ...
    print("canvas assigned: " + str(canvas))
    return canvas

def canvas_quit():
    print("canvas_quit called")
    app.quit()
    exit()